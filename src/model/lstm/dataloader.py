...
class LSTMDataLoader():
    ...
    def make_dataset(self, train_size: float, train_batch_size: int, valid_batch_size: int, seq_length: int) -> tuple[DataLoader, torch.FloatTensor, torch.FloatTensor]:
        ...
        train_data_loader = DataLoader(dataset=train_dataset, batch_size=train_batch_size, shuffle=True) # drop_last=True
        valid_data_loader = DataLoader(dataset=valid_dataset, batch_size=valid_batch_size, shuffle=True) # drop_last=True
